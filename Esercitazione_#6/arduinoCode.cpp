...
int D_out, N_B, pin, sleepTime;
float R_2, R_3, S, T_C, T_K, V_out, V_FR;

void setup() {
  Serial.begin(9600); // setup Serial
  pin = A0; // set the pin to read data
  sleepTime = 1000;  // set how much time the sensor must execute the measurement of the temperature
  S = 10 * pow(10, -3);
  N_B = 10;
  R_2 = 3.8 * pow(10, 3);
  R_3 = 1 * pow(10, 3);
...
void loop() {
  ...
  T_K = (float) D_out * V_FR / pow(2, N_B) * 1 / S;
  V_out = (float) D_out * V_FR / pow(2, N_B);
#ifdef INT
  T_K = T_K * (1 + R_2 / R_3);
