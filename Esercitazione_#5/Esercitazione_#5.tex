...
\begin{document}
	...
	\section{Esperienza in laboratorio}
		...
		\subsection{Amplificatore invertente}
			...
			% foto 2 e foto 3 affiancate
			% foto 4 con commento $ V_{\mathrm{pp}} = 5.5 \, \mathrm{V} $
		\subsection{Amplificatore differenziale}
			...
			\begin{center}
				...
				\end{tabular}
			\end{center}
			Infine, abbiamo impostato il generatore di segnali, in modo da visualizzare un segnale sinusoidale con $ V_{\mathrm{pp}} $ pari ad $ 1.6 \, \mathrm{V} $ e frequenza pari a $ 200 \, \mathrm{Hz} $, e proceduto con la misurazione di $ V_{\mathrm{i}} $ e $ V_{\mathrm{u}} $ tramite l'uso dell'oscilloscopio.
		\subsection{Amplificatore AC/DC}
			...
			\begin{center}
				...
				\end{tabular}
			\end{center}
			Successivamente, abbiamo impostato il generatore di segnali, in modo da visualizzare un segnale triangolare con $ V_{\mathrm{pp}} $ pari ad $ 300 \, \mathrm{mV} $, e proceduto con la misurazione di $ V_{\mathrm{i}} $ e $ V_{\mathrm{u}} $ tramite l'uso dell'oscilloscopio.
	%-----------------------------------------------------------------------------
	%  RESULTS
	%-----------------------------------------------------------------------------
	\section{Risultati}
		...
		\subsection{Amplificatore differenziale}
			(Dopo ogni misurazione il circuito è stato riposizionato alla situazione di partenza)
			%inizio elenco
			* Chiudendo S8 creiamo una maglia priva di resistenze, il generatore è direttamente collegato al morsetto non invertente, infatti V2=Vi, di conseguenza la loro differenza è pari a 0 ed il guadagno che avremo sarà unitario, ovvero la tensione che entra nell'amlificatore resta invariata.
			\begin{equation*}
				\begin{split}
				...
				\end{split}
			\end{equation*}
			* Chiudendo S9 creiamo una maglia con R6 = 10kOhm, di conseguenza avremo una diminuzione di V2  =2/3Vi, che porterà ad un incremento del guadagno in modulo, come riscontrabile anche dai calcoli effettuati.
			\begin{equation*}
				\begin{split}
				...
				\end{split}
			\end{equation*}
			* Chiudendo S10 creiamo una maglia con R6 = 10kOhm e R7 = 10kOhm, di conseguenza avremo una diminuzione di V2  =1/3Vi, che porterà ad un incremento del guadagno in modulo, come riscontrabile anche dai calcoli effettuati.
			\begin{equation*}
				\begin{split}
				...
				\end{split}
			\end{equation*}
			* Chiudendo S11 colleghiamo direttamente il morsetto non-invertenta alla massa, quindi V2  =0, otteniamo il circuito di un amplificatore invertente, con guadagno massimo
			\begin{equation*}
				\begin{split}
			...
			\begin{center}
				...
				\end{tabular}
			\end{center}
			Le misurazioni effettuate sono coerenti con i valori calcolati, infatti al massimo troviamo differenze di un decimo del valore, ma tendenzialmente la differenza è molto minore.
		\subsection{Amplificatore AC/DC}
			Punto a)
				Abbiamo osservato, per mezzo dell'oscilloscopio, che al crescere della frequenza il segnae veniva distorto a causa della limitazione di slew rate %foto 5
				di conseguenza abbiamo continuato ad aumentare la frequenza in ingresso come richiesto, riducendo però la Vpp, partendo da un valore di 1,6V per scendere fino a 300 mV, rimanendo, quindi, nei limiti richiesti.
				%foto 6
				%inserire tabella
			Punto b)	
				Calcolando la frequenza di taglio abbiamo ottenuto f=86.08kHz???????????
			Punto c)
				Per verificare che l'offset vega amplificato lo abbiamo inserito pari a 140mV (foto 9), per poi aumentarlo fino a 180 mV (foto 10) ed apprezzare la differente rappresentazione dell'offset, sono stati mantenuti valori bassi per evitare distorsioni del segnale.
				Il segnale in uscita presenta l'offset anch'esso amplificato di un fattore Av.
			Punto d)
				(Premessa: il valore misurato di C3è 15pF, non 10nF come riportato nella consegna)
				Abbiamo inserito C3 come da istruzioni ed abbiamo ricalcolato la frequenza di taglio, ottenendo f = 54kHz, quindi 32kHz di calo. 
				...
				%foto 12 e 13
				Variazioni di offset per misurare il guadagno in continua
			Punto e)
				Inserendo il condensatore C4= 100 nF ed impostando Vi= 360 mV otteniamo Vu=2,86 V ed una nuova frequenza di taglio f = 83kHz, cioè inferiore di 3 kHz rispetto a quella del circuito privo di condensatori. 
				%foto 19
			Punto f)
				Abbiamo inserito C5, Vi invariata, Vu= 2,86 V, quindi il condensatore non influenza la tensione di ucita, e frequenza di taglio f= 84kHz.
				%foto 14
